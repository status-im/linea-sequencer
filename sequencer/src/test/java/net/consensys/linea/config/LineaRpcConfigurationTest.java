...
class LineaRpcConfigurationTest {
  ...
  void builder_withDefaultValues_shouldBuildCorrectly() {
    ...
    LineaRpcConfiguration config =
        LineaRpcConfiguration.builder()
            .sharedGaslessConfig(sharedConfig) // Minimum required for builder
            .build();

    assertFalse(
        config.estimateGasCompatibilityModeEnabled(),
        "Default estimateGasCompatibilityModeEnabled should be false");
    assertEquals(
        BigDecimal.valueOf(1.2),
        config.estimateGasCompatibilityMultiplier(),
        "Default estimateGasCompatibilityMultiplier");
    assertFalse(
        config.gaslessTransactionsEnabled(), "Default gaslessTransactionsEnabled should be false");
    assertEquals(1.5, config.premiumGasMultiplier(), 0.001, "Default premiumGasMultiplier");
    assertFalse(
        config.allowZeroGasEstimationForGasless(),
        "Default allowZeroGasEstimationForGasless should be false");
    assertSame(
        sharedConfig, config.sharedGaslessConfig(), "Shared config should be the one provided");
  }

  @Test
  void builder_withSpecificValues_shouldBuildCorrectly() {
    LineaSharedGaslessConfiguration sharedConfig =
        new LineaSharedGaslessConfiguration("/custom/deny.list", 120L, 20L, 30L);
    LineaRpcConfiguration config =
        LineaRpcConfiguration.builder()
    ...
    assertEquals(
        "/custom/deny.list",
        actualSharedConfig.denyListPath()); // Check pass-through by accessing shared config first
  }
  ...
  void denyListPath_accessor_shouldReturnPathFromSharedConfig() {
    ...
    LineaSharedGaslessConfiguration shared =
        new LineaSharedGaslessConfiguration(expectedPath, 60L, 10L, 10L);
    LineaRpcConfiguration config =
        LineaRpcConfiguration.builder().sharedGaslessConfig(shared).build();
  ...
  void denyListRefreshSeconds_accessor_shouldReturnRefreshFromSharedConfig() {
    ...
    LineaSharedGaslessConfiguration shared =
        new LineaSharedGaslessConfiguration("path", expectedRefresh, 10L, 10L);
    LineaRpcConfiguration config =
        LineaRpcConfiguration.builder().sharedGaslessConfig(shared).build();
  ...
  void premiumGasPriceThresholdGWei_accessor_shouldReturnThresholdFromSharedConfig() {
    ...
    LineaSharedGaslessConfiguration shared =
        new LineaSharedGaslessConfiguration("path", 60L, expectedThreshold, 10L);
    LineaRpcConfiguration config =
        LineaRpcConfiguration.builder().sharedGaslessConfig(shared).build();
  ...
  void denyListEntryMaxAgeMinutes_accessor_shouldReturnMaxAgeFromSharedConfig() {
    ...
    LineaSharedGaslessConfiguration shared =
        new LineaSharedGaslessConfiguration("path", 60L, 10L, expectedMaxAge);
    LineaRpcConfiguration config =
        LineaRpcConfiguration.builder().sharedGaslessConfig(shared).build();

 ...
 * @param rlnProofLocalWaitTimeoutMs Timeout for waiting for a proof in local cache during validation (in ms).
 * @param sharedGaslessConfig Shared configuration including the deny list path and premium gas threshold.
 * @param karmaServiceUrl Optional URL to an HTTP service that provides user karma/tier, quota, and epoch usage.
...
public record LineaRlnValidatorConfiguration(
  ...
  public long premiumGasPriceThresholdWei() {
      return sharedGaslessConfig.premiumGasPriceThresholdGWei() * 1_000_000_000L; // Convert GWei to Wei
  }

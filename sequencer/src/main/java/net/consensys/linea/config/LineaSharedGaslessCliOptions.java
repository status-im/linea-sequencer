...
import net.consensys.linea.plugins.LineaCliOptions; // Import the interface
import picocli.CommandLine;

public class LineaSharedGaslessCliOptions implements LineaCliOptions { // Implement the interface
    ...
    @CommandLine.Option(
            ...
            arity = "1")
    private long denyListRefreshSeconds = LineaSharedGaslessConfiguration.DEFAULT_DENY_LIST_REFRESH_SECONDS;

    @CommandLine.Option(
            names = "--linea-shared-premium-gas-threshold-gwei",
            description = "Premium gas price threshold in GWei for bypassing deny list (default: ${DEFAULT-VALUE})",
            arity = "1")
    private long premiumGasPriceThresholdGWei = LineaSharedGaslessConfiguration.DEFAULT_PREMIUM_GAS_PRICE_THRESHOLD_GWEI;

    @CommandLine.Option(
            names = "--linea-shared-deny-list-max-age-minutes",
            description = "Maximum age in minutes for a deny list entry before it expires (default: ${DEFAULT-VALUE})",
            arity = "1")
    private long denyListEntryMaxAgeMinutes = LineaSharedGaslessConfiguration.DEFAULT_DENY_LIST_ENTRY_MAX_AGE_MINUTES;

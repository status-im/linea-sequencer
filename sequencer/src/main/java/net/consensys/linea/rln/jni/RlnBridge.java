...
public class RlnBridge {
    ...
    static {
        ...
        try {
            ...
            System.out.println("Native library '" + LIBRARY_NAME + "' loaded successfully via System.loadLibrary.");
            loaded = true;
            ...
            String libDirGuess1 = Paths.get("src", "main", "rust", "rln_bridge", "target", "release").toAbsolutePath().toString();
            String libDirGuess2 = Paths.get("build", "native", "release").toAbsolutePath().toString(); // Common for Gradle native builds
            ...
            try {
                ...
                try {
                    ...
                    System.err.println("\nPlease ensure:");
                    System.err.println("  1. You have compiled the Rust code (e.g., `cd sequencer/src/main/rust/rln_bridge && cargo build --release`)");
                    System.err.println("  2. The library file ('" + mappedLibName + "') exists at one of the attempted paths or is in java.library.path.");
                    System.err.println("  3. The library architecture matches your JVM architecture.");
                    System.err.println("  4. If using a build system like Gradle, ensure it correctly packages/locates the native library.");
        ...
        if (!loaded) {
            ...
            throw new UnsatisfiedLinkError("Could not load native library '" + LIBRARY_NAME + "' by any method.");
        }
    }
     ...
     * @param publicInputsHex Array of public input hex strings in order: [share_x, share_y, epoch, root, nullifier].

 ...
 * <ul>
 *   <li>Channel management with proper TLS configuration</li>
 *   <li>Request timeout handling</li>
 *   <li>Comprehensive error handling for various gRPC failure scenarios</li>
 *   <li>Resource cleanup and connection lifecycle management</li>
 * </ul>
 * 
 * <p>Used by both RLN validation and gas estimation components to avoid
 * code duplication in karma service interactions.
 * 
...
public class KarmaServiceClient implements Closeable {
   ...
   */
  public record KarmaInfo(String tier, int epochTxCount, int dailyQuota, String epochId, long karmaBalance) {}
   ...
   */
  public KarmaServiceClient(String serviceName, String host, int port, boolean useTls, long timeoutMs) {
   ...
   */
  public KarmaServiceClient(String serviceName, String host, int port, boolean useTls, long timeoutMs, ManagedChannel providedChannel) {
    ...
    if (providedChannel != null && !providedChannel.isShutdown()) {
      ...
      LOG.info("{}: Creating new ManagedChannel for Karma Service client at {}:{}", serviceName, host, port);
    ...
    this.stub = KarmaServiceGrpc.newBlockingStub(this.channel)
  ...
  public Optional<KarmaInfo> fetchKarmaInfo(Address userAddress) {
    ...
    try {
      LOG.debug("{}: Fetching karma info for user {} via gRPC", serviceName, userAddress.toHexString());
      KarmaResponse response = stub.getKarma(request);

      LOG.debug("{}: Karma service response for {}: tier={}, epochTxCount={}, dailyQuota={}, epochId={}, karmaBalance={}",
               serviceName, userAddress.toHexString(), response.getTier(), response.getEpochTxCount(),
               response.getDailyQuota(), response.getEpochId(), response.getKarmaBalance());
      ...
      if (code == Status.Code.NOT_FOUND) {
        ...
        LOG.error("{}: Karma service gRPC error for user {}: {}", 
                 serviceName, userAddress.toHexString(), e.getMessage(), e);
        return Optional.empty();
      }
    } catch (Exception e) {
